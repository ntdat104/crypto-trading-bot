...
def generate_signals(df, threshold=1.5):
    df['signal'] = 0
    df['signal'][df['z_score'] < -threshold] = 1  # Buy signal
    df['signal'][df['z_score'] > threshold] = -1  # Sell signal
...
def plot_results(df):
    ...
    plt.plot(df.index, df['rolling_mean'] + 1.5 * df['rolling_std'], label='Mean + Threshold', linestyle='--', alpha=0.7)
    plt.plot(df.index, df['rolling_mean'] - 1.5 * df['rolling_std'], label='Mean - Threshold', linestyle='--', alpha=0.7)
    ...
    plt.title('BTC/USDT Mean Reversion Strategy')
    plt.legend()
    plt.show()
...
# Calculate indicators, generate signals, simulate trading, and plot results
